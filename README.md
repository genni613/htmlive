...
Use **Browse page** (or press `Space`) before following links. A full navigation removes any bookmarklet from the old document, so launch HTMLive again on the destination page. A future Chrome extension can perform that injection automatically.
...
跳转前可点击 **浏览页面**（或按 `Space`），此时页面链接可以正常使用。完整页面跳转会移除旧页面中注入的书签代码，因此到达新页面后需再次点击 HTMLive 书签；未来做成 Chrome Extension 后可自动完成注入。
