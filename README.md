...
- Apply the latest text, style, or attribute edit to selected same-origin static pages and export them as a ZIP
...
- Restore the latest batch patch and page choices from the same tab after navigating and launching the bookmarklet again
- No build step or application backend—the editor runs in the page

### Batch static pages

After making a text, inline-style, or attribute change, choose **Batch pages**. HTMLive discovers same-origin links from the current page, lets you add more same-origin URLs, previews selector matches page by page, and downloads the successfully matched HTML documents as a ZIP.

Batch export reads static HTML responses. For local files, serve the folder over HTTP during editing (for example with the local development command below) so Chrome can read linked pages. Client-rendered routes and API-backed pagination are outside this mode.

Use **Browse page** (or press `Space`) before following links. A full navigation removes any bookmarklet from the old document, so launch HTMLive again on the destination page; the latest batch patch and page choices are restored from that tab's session.
...
examples/multi-page-site/     Static multi-page fixture for batch export testing
...
- 导出为独立 HTML 文件
- 将最近一次文字、样式或属性修改同步到选中的同域静态页面，并导出 ZIP
- 使用“浏览页面”模式正常操作链接和控件；`⌘/Ctrl + 点击`链接始终不会触发选取
- 同一标签页跳转并重新启动书签后，恢复最近的批量补丁和页面选择
- 无构建步骤、无应用后端：编辑器直接在当前页面运行

### 批量静态页面

完成一次文字、内联样式或属性修改后，点击 **批量页面**。HTMLive 会发现当前页中的同域链接，也支持手动补充同域 URL；确认逐页匹配结果后，可将成功匹配的 HTML 页面打包为 ZIP 下载。

批量导出读取的是静态 HTML 响应。本地文件需要先通过 HTTP 启动（例如使用下方的本地开发命令），Chrome 才能读取互相链接的页面；客户端动态渲染路由和接口分页暂不属于此模式。

跳转前可点击 **浏览页面**（或按 `Space`），此时页面链接可以正常使用。完整页面跳转会移除旧页面中注入的书签代码，因此到达新页面后需再次点击 HTMLive 书签；同一标签页会自动恢复最近的批量修改和页面选择。
