...
- 一键将本次会话发现的全部同域 HTML 页面导出为 ZIP，并分别应用每一页自己的修改
...
点击 **导出全部页面** 后，HTMLive 会读取本次会话发现的所有同域 HTML 页面，分别应用对应 URL 保存的修改。Chrome 会先让你选择 ZIP 的保存位置；不支持保存窗口的浏览器会使用默认下载目录。压缩包目前包含 HTML 文档，并保留原有 CSS、图片和脚本的相对引用；资源文件本身暂不复制进压缩包。
