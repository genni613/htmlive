...
Choose **Export all pages** to fetch every same-origin HTML page discovered during the session, apply that URL's saved edits, and download one ZIP. The archive contains HTML documents; their existing relative CSS, image, and script references are preserved, but those assets are not copied into the archive yet.
...
点击 **导出全部页面** 后，HTMLive 会读取本次会话发现的所有同域 HTML 页面，分别应用对应 URL 保存的修改，然后下载一个 ZIP。压缩包目前包含 HTML 文档，并保留原有 CSS、图片和脚本的相对引用；资源文件本身暂不复制进压缩包。
